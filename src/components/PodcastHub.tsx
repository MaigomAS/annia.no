...
import { youtubeChannel } from '../data/podcasts'
...
export function PodcastHub({ compact = false }: { compact?: boolean }) {
  const { t } = useLanguage()
  const podcastIntent = [t('podcasts.intents.first'), t('podcasts.intents.second'), t('podcasts.intents.third')]
  ...
  return (
    <section className={compact ? 'mx-auto max-w-7xl px-5 py-14 lg:px-8' : 'mx-auto max-w-7xl px-5 py-20 lg:px-8'}>
      <div className="grid gap-10 lg:grid-cols-[.82fr_1.18fr] lg:items-end">
        <SectionHeader eyebrow={t('podcasts.eyebrow')} title={t('podcasts.title')} description={t('podcasts.description')} />
      ...
      <GlassPanel className="mt-10 overflow-hidden p-4 md:p-6">
        {status === 'loading' ? (
          <div className="flex min-h-96 flex-col items-center justify-center gap-4 text-center text-steel"><Loader2 className="h-8 w-8 animate-spin text-cyanMist" /><p>{t('podcasts.loading')}</p></div>
        ) : null}
        ...
        {status === 'error' || status === 'empty' ? (
          <div className="grid min-h-96 place-items-center rounded-[1.5rem] border border-dashed border-white/15 bg-white/[0.03] p-8 text-center">
            <div className="max-w-2xl"><Rss className="mx-auto h-10 w-10 text-cyanMist" /><h2 className="mt-5 text-2xl font-semibold text-bone">{t('podcasts.fallbackTitle')}</h2><p className="mt-3 leading-7 text-steel">{t('podcasts.fallbackDescription', { handle: youtubeChannel.handle })}</p><div className="mt-6"><a href={youtubeChannel.videosUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/8 px-5 py-3 text-sm font-semibold text-bone transition hover:border-cyanMist/50 hover:bg-white/12">{t('podcasts.openYouTube')} <ExternalLink className="h-4 w-4" /></a></div></div>
          </div>
        ) : null}
        ...
        {activeEpisode && embedUrl ? (
          <div className="grid gap-6 lg:grid-cols-[1.18fr_.82fr]">
            ...
            <div className="flex flex-col justify-between rounded-[1.5rem] border border-white/10 bg-white/[0.045] p-6">
              <div><p className="text-xs uppercase tracking-[0.24em] text-cyanMist">{t('podcasts.featuredPodcast')}</p><h2 className="mt-3 text-2xl font-semibold text-bone">{activeEpisode.title}</h2><p className="mt-4 leading-7 text-steel">{cleanDescription(activeEpisode.description, t('podcasts.defaultEpisodeDescription'))}</p><p className="mt-4 text-sm text-arctic">{formatDate(activeEpisode.publishedAt, t('podcasts.locale'), t('podcasts.latestEpisode'))}</p></div>
              <div className="mt-8 flex flex-wrap items-center gap-3"><a href={activeEpisode.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full bg-cyanMist px-5 py-3 text-sm font-semibold text-midnight transition hover:bg-bone"><Play className="h-4 w-4" /> {t('podcasts.watchOnYouTube')}</a><a href={youtubeChannel.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-white/10 px-5 py-3 text-sm font-semibold text-bone transition hover:border-cyanMist/60 hover:text-cyanMist"><ExternalLink className="h-4 w-4" /> {t('podcasts.subscribe')}</a></div>
            </div>
          </div>
        ) : null}
